...
export default function CurrentWeatherCardModal({currentWeatherData,unit}:CurrentWeatherCardModalTypes){
    ...
    return(
        ...
        <div className="w-full max-w-5xl flex flex-col gap-2 lg:flex-row justify-between p-5 bg-transparent text-slate-900 dark:text-slate-200">
            
            <p className="text-xl text-center lg:hidden">{dayName} {languageDictionary.currentweather.cardtitlehourly}</p>

            <div className="flex flex-col items-center lg:flex-row gap-4 lg:gap-5">

                <div className="flex flex-row items-center gap-10 lg:gap-5">
                    <img className="w-30 h-30 lg:w-40 lg:h-40 object-contain" src={iconPath} alt="current weather image"></img>

                    <div className="flex flex-row items-center justify-center">
                        <p className="text-7xl lg:text-7xl font-bold leading-none p-0">{UnitConverter(Math.round(currentWeatherData.temp),unit)}</p>
