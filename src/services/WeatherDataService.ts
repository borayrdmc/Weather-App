...
export const WeatherDataService= async (lat:number,lon:number): Promise<WeatherDataTypes>=>{
    ...
    const response = await fetch(`${BASE_URL}/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely,alerts&units=metric&appid=${API_KEY}`,{next:{revalidate:3600}}); //Cache for 1 hour
